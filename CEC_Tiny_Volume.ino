...
#define ADDRESS 0x05  //pretend to be an audio system
#define CECPIN 3      // CEC on Pin2 on ATTINY25
#define ONOFFPIN 0    // Output ON/OFF on Pin5 on ATTINY25
#define VOLUPPIN 1    // Output VOL-UP on Pin6 on ATTINY25
#define VOLDNPIN 2    // Output VOL-DN on Pin7 on ATTINY25
#define ACTIVE 1      // Set to 1 to reply to CEC messages
#define NORESISTORS 1 // Set to 1 if using internal pull-ups to drive LEDs
...
void setup() {
  ...
  OSCCAL=108; // Tune this when using internal ocsilator, as CEC is timing-sensitive
}
  ...
  if ((pldcnt > 1)) {
    switch (pld[1]) {
      ...
      case 0x8f:
        ...
        break;
      case 0x44:
        if (pld[2] == 0x41) { //VOL UP PRESSED
          digitalWrite(VOLUPPIN, HIGH);
        }
        else if (pld[2] == 0x42) { //VOL DOWN PRESSED
          digitalWrite(VOLDNPIN, HIGH);
        }
        break;
      case 0x45:
        digitalWrite(VOLUPPIN, LOW); //VOL RELEASED
        digitalWrite(VOLDNPIN, LOW); //VOL RELEASED
        break;
      default:
        break;
    }
